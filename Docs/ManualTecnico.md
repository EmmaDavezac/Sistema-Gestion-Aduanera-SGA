...
> **Autor:** Luciano Emmanuel Davezac  
...
**Ubicación:** `apps/SGA/management/commands/enviar_alertas.py`

Comando Django que implementa el requisito RF34: envío automático de alertas por correo electrónico a todos los usuarios activos del sistema cuando existen exportaciones próximas a vencer o ya vencidas sin oficializar.

#### Lógica del comando

1. Calcula la fecha límite: `hoy + 5 días`.
2. Consulta exportaciones que cumplan las tres condiciones simultáneamente:
   - `vencimiento_preimposicion <= hoy + 5 días`
   - `oficializacion` es `NULL` (no oficializadas)
   - `baja = False`
3. Si no hay alertas, termina sin enviar correos.
4. Obtiene todos los usuarios activos con email configurado.
5. Genera y envía un correo HTML a cada usuario con la lista de operaciones críticas, diferenciando visualmente entre **vencidas** (fondo rojo) y **por vencer** (texto naranja).
6. Usa una única conexión SMTP para enviar todos los mensajes en lote (`send_messages`).

#### Ejecución manual

```bash
# Dentro del contenedor backend
docker exec -it sga_backend python manage.py enviar_alertas

# En instalación manual (con entorno virtual activo)
python manage.py enviar_alertas
```

#### Ejecución automática — Cron en Docker

El comando se ejecuta automáticamente mediante **cron** configurado en el `Dockerfile` del backend:

```dockerfile
# Instala cron en la imagen
RUN apt-get update && apt-get install -y cron

# Configura la tarea: todos los días a las 8:00 AM hora Argentina (11:00 UTC)
...
# Al iniciar el contenedor: exporta variables de entorno para que cron las lea,
# luego arranca cron y el servidor Django en paralelo
CMD printenv | grep -v "no_proxy" >> /etc/environment && cron && python manage.py runserver 0.0.0.0:8000
```

> **Nota sobre variables de entorno:** el comando `printenv >> /etc/environment` es necesario porque cron no hereda las variables de entorno del contenedor Docker. Sin este paso, el comando no tendría acceso a `EMAIL_HOST_USER`, `EMAIL_HOST_PASSWORD` ni `SECRET_KEY`.

#### Logs

Los resultados de cada ejecución se registran en `/var/log/cron.log` dentro del contenedor:

```bash
# Ver logs del cron en tiempo real
docker exec -it sga_backend tail -f /var/log/cron.log
```

#### Horario configurado

| Entorno | Expresión cron | Hora local (Argentina) |
|---------|---------------|------------------------|
| Producción | `0 11 * * *` | 8:00 AM todos los días |
| Pruebas | `*/5 * * * *` | Cada 5 minutos |

> Para cambiar entre modos editá el `Dockerfile` del backend: comentá la línea de producción y descomentá la de pruebas (o viceversa), luego reconstruí con `docker-compose up --build`.
...
Todas las variables de entorno se centralizan en un único archivo `.env` ubicado en la **raíz del proyecto** (junto a `docker-compose.yml`). Docker Compose lo lee automáticamente sin configuración adicional.
...
#### `.env.example` — raíz del proyecto (versionar este, nunca el `.env`)
...
|----------|----------|-------------|-----------|
| `SECRET_KEY` | Backend | Clave secreta de Django. Debe ser única y aleatoria | Sí |
| `DEBUG` | Backend | `True` en desarrollo, `False` en producción | Sí |
| `ALLOWED_HOSTS` | Backend | Hosts permitidos, separados por coma | Sí |
| `EMAIL_HOST_USER` | Backend | Correo Gmail para envío de alertas de vencimiento | No* |
| `EMAIL_HOST_PASSWORD` | Backend | Contraseña de aplicación de Gmail (no la contraseña personal) | No* |
| `VITE_API_URL` | Frontend | URL base del backend Django | Sí |

> *Sin `EMAIL_HOST_USER` y `EMAIL_HOST_PASSWORD` el sistema funciona normalmente pero no envía correos de alerta. Para Gmail se debe generar una **contraseña de aplicación** desde la configuración de seguridad de la cuenta Google.
...
El archivo `.env.example` **sí debe** versionarse — sirve como referencia para otros desarrolladores.

---

#### Correcciones de código necesarias

**`docker-compose.yml`** — reemplazar los valores hardcodeados por referencias a las variables del `.env`:

```yaml
services:
  backend:
    build: ./Backend
    container_name: sga_backend
    volumes:
      - ./Backend:/app
    ports:
      - "8000:8000"
...
> Docker Compose lee el `.env` de la raíz automáticamente y sustituye los `${}` al levantar los contenedores.

**`backend/settings.py`** — reemplazar los valores hardcodeados por `os.environ.get()`:
...
**`frontend/src/api/axios.js` y `frontend/src/api/api.js`** — reemplazar la URL hardcodeada:
...
> **Nota:** El `docker-compose.yml` original usaba `REACT_APP_API_URL`, que es la convención de Create React App. Este proyecto usa **Vite**, donde las variables expuestas al cliente deben comenzar con `VITE_`. El nombre correcto es `VITE_API_URL`.
...
**2. Crear el archivo `.env`** en la raíz copiando el ejemplo y completando los valores:
